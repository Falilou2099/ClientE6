...
class AuthController {
    ...
    public function processLogin() {
        ...
        $appType = $_POST['app_type'] ?? 'light'; // 'light' pour l'application web, 'heavy' pour l'application Java

        $result = $this->login($email, $password, $appType);
        ...
        if ($result['success']) {
            ...
            header('Location: /bigpharma/public');
            exit();
        } else {
    ...
    public function processRegister() {
        ...
        $data = [
            ...
            'app_access' => $_POST['app_access'] ?? 'both'
        ];
    ...
    public function registerPharmacy($data) {
        ...
        // Déterminer le type d'accès à l'application
        $appAccess = $data['app_access'] ?? 'both';
        ...
        // Créer un utilisateur pour cette pharmacie
        $user = User::register(
            $data['email'],
            $data['password'],
            $pharmacy->getId(),
            'admin', // Rôle par défaut pour le propriétaire de la pharmacie
            $data['app_access'] // Type d'accès à l'application
        );
    ...
    // Connexion
    public function login($email, $password, $appType = 'light') {
        ...
        // Vérifier si l'utilisateur a accès à ce type d'application
        $appAccess = $user->getAppAccess();
        if ($appAccess !== 'both' && $appAccess !== $appType) {
            return [
                'success' => false,
